...
half4 UnlitPassFragment(Varyings input) : SV_Target
{
    ...
    InitializeIceData(input, inputData);//计算基础信息，计算观察向量的世界空间和切线空间的内容再加上 世界空间最后的法线信息，因为有两张法线（一个自身的法线，一个细节法线），对其进行混合。
    half4 finalColor = 1;

    CombinedEffects(input, inputData, finalColor.rgb); //计算颜色

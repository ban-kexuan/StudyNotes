...
void EffectNormal(float3x3 t2w, float2 uv, out half3 normalTS, out half3 normalWS)
{
    ...
    normalTS.rg *= _GlassNormalScale;//如果要给法线乘一个强度，要保证b通道是1，只对rg通道乘，乘完要normalize
    normalTS = normalize(normalTS);
    
    #ifdef _USEDETAILNORMAL
    half3 detailNormalTS = UnpackNormal(tex2D(_DetailNormal, uv * _GlassDetailUVScale));//细节法线就没有用normalize
    normalTS = BlendAngleCorrectedNormals(normalTS, detailNormalTS);//将两者的法线进行混合
    normalTS = normalize(normalTS);
    #endif // _USEDETAILNORMAL
    
    #endif // _USEGLASSNORMAL
    
    normalWS = TransformTangentToWorld(normalTS, t2w);//转到世界空间
}
...
void InitializeIceData(Varyings input, out IceInputData inputData)
{
    ...
    inputData.viewDirectionWS = normalize(float3(input.normalWS.w, input.tangentWS.w, input.bitangentWS.w));//viewdir是这么实现的
...
half3 EffectDust(Varyings input, IceInputData inputData)
{
    ...
    // 视觉偏移增加 _DustDepthShift 参数作为 k 系数，这里使用切线空间的视图方向，确保尘埃的效果与法线方向一致
    ...
    // FinalColor
    float3 finalColor = color1 * color1 + color2 * color2;//相乘再相加增加对比度
...
void IntersectionMesh(float3 rayOrigin, float3 rayDirection, float radius, IceInputData inputData, out float thickness)
{
    ...
    float3 oc = rayOrigin - offsetPosWS;//起始点就是摄像机，终点是球心
    float b = dot(rayDirection, oc);//观察方向 点乘 摄像机到球心
    float c = dot(oc, oc) - radius * radius;
    float h = b * b - c;
    h = sqrt(h);
    
    thickness = h * 2; //总之这里就是得到厚度
}

// 获取物体缩放值
float3 ObjectScale()
{
    //通过将单位向量转到世界空间下得到物体的缩放值
    float3 output = 0;
...
half4 DiffuseEffect(Varyings input, IceInputData inputData)
{
    ...
    thickness = max(0, thickness); //得到物体的厚度
...
void CombinedEffects(Varyings input, IceInputData inputData, out half3 color)
{
    ...
    // Reflection
    half4 reflection = EffectReflection(inputData);//用菲涅尔计算反射
